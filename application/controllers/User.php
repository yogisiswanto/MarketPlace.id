...
class User extends CI_Controller {
	...
	public function testHotp()
	{
		...
		$hotp = $this->hotp->make('"N�`\+�7����Y�', 1);
		debug($hotp);
	}

	public function testLcg()
	{
		$key = $this->lcg->random("081299999999");
		debug($key);
	}
	...
	public function testRC6()
	{
		...
		$plaintext = '20101010101010';

		// getting keyScheduling from key
		$keyScheduling = $this->rc6->keySchedule(1218693);
		...
		for($i=0; $i < strlen($cipherText); $i++) { 
			...
			$hexadecimal = $hexadecimal.dechex(ord($cipherText[$i]))." ";
		}
		echo "<br/>";
		...
		$hotp = $this->hotp->make($cipherText, 1);
		...
		$data = array(
					...
					'HOTP'				=> $hotp,
					'time executed'		=> $this->benchmark->elapsed_time('code_start', 'code_end'),
				);
	...
	function testAvalancheEffect()
	{
		...
		// for($i=0; $i < strlen($modifiedCipherText); $i++) { 
			
		// 	$modifiedhexadecimal = $modifiedhexadecimal.dechex(ord($modifiedCipherText[$i]))." ";
		// }
	...
	public function testOTP()
	{
		...
		// update seed with new value from lcg
		$this->session->set_userdata('seed', $key['X']);

		// getting plaintext from date (YYYYmmddHis)
		$plaintext = $this->date->get();

		// getting keyScheduling from key
		$keyScheduling = $this->rc6->keySchedule($key['X']);
		...
		$data['otp'] = $otp['code'];
		$data['HMAC'] = $otp['HMAC'];
		$data['execution'] = $this->benchmark->elapsed_time('code_start', 'code_end');
